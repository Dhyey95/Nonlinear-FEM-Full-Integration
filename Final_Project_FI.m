...
for i = 1:N_load_pt 
            ...
            for m = 1:nele
                ...
                for cc = 1:Np2
                    for pp = 1:Np2
                        ...
                        f_internal = f_internal + B'*Sigma*det(dum_J)*w2(cc)*w2(pp);
                        K_local = K_local + B'*(D+T)*B*det(dum_J)*w2(cc)*w2(pp);
                    end
                end
    ...
    show = ['Loadstep ',num2str(i),' has converged in ',num2str(count),' iterations.'];
    disp(show)
...
defl_x_FI = delta_defl(1:2:end);
defl_y_FI = delta_defl(2:2:end);
dof_half_y = n_x*(ele_y/2)+1:1:n_x*(ele_y/2 +1);
defl_half_y = defl_y_FI(dof_half_y);
...
title('Load vs Deflection with Full Integration')

figure(2)
plot(coord_half_x, defl_half_y)
xlabel('x-cordinate')
ylabel('Deflection')
title('Deflection along X-direction @ y = 0')
figure(3)
plot(corr_coord_half_x, sigma_xx)
xlabel('y-coordinate')
ylabel('Sigma XX in kips')
title('Sigma XX across x = L/2 with FI')

figure(4)
plot(corr_coord_half_x, sigma_xy)
xlabel('y-coordinate')
ylabel('Sigma XY in kips')
title('Sigma XY across x = L/2 with FI')


figure(5)
plotbeam(defl_x_FI,defl_y_FI,ele_x,ele_y)
